...
static CMOCK_MEM_INDEX_TYPE CMock_Guts_BufferSize =
    CMOCK_MEM_SIZE + CMOCK_MEM_ALIGN_SIZE; // sizeof(CMock_Guts_Space);
static CMOCK_MEM_INDEX_TYPE CMock_Guts_FreePtr = CMOCK_MEM_ALIGN_SIZE;
#endif
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemNew(CMOCK_MEM_INDEX_TYPE size) {
    ...
    /* verify arguments valid (we must be allocating space for at least 1 byte, and the existing
     * chain must be in memory somewhere) */
    if (size < 1) {
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemChain(CMOCK_MEM_INDEX_TYPE root_index,
    ...
    if (root_index == CMOCK_GUTS_NONE) {
        ...
        *(CMOCK_MEM_INDEX_TYPE *)((CMOCK_MEM_PTR_AS_INT)next - CMOCK_MEM_INDEX_SIZE) =
